...
const SpotifyNowPlaying = () => {
  ...
  if (!data || (!data.isPlaying && !data.error)) {
    return (
      <div 
        className="fixed bottom-6 right-6 z-40 rounded-xl p-3 cursor-pointer transition-all duration-300 hover:bg-white/[0.05] bg-white/[0.03] border border-white/[0.08] backdrop-blur-md"
        title="Nepřehrává se"
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-white/50">Spotify</span>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div 
        className="overflow-hidden transition-all duration-300 bg-white/[0.03] border border-white/[0.08] rounded-xl backdrop-blur-md"
      >
        {isExpanded ? (
          <a 
            href={data.songUrl} 
